...
- [[#sigrok-python-api-demo][Sigrok Python API Demo]]
  ...
  - [[#toggle-rdtech-dps5020-enabled-state][Toggle RDTech DPS5020 enabled state]]
...
[[http://sigrok.org/gitweb/?p=libsigrok.git;a=tree;f=bindings/python][Sigrok Python bindings]] is a thin wrapper on top of [[http://sigrok.org/wiki/Libsigrok][libsigrok]] C++
...
~--show~ -command for the string values. For example, for the demo
-device
...
Channel groups:
    ...
    Analog: channels A0 A1 A2 A3 A4
    A0: channel A0
    A1: channel A1
    A2: channel A2
    A3: channel A3
...
Again, Use ~sigrok-cli~ ~--show~ -command for the configuration option names. For
...
** Toggle RDTech DPS5020 ~enabled~ state
...
#+BEGIN_SRC python :eval no-export :results output :noweb no :session *Python* :var reset=reset-modules ramp_up_module=ramp_up_module :exports both
...
because driver reports error messages (ref chapter 'Run two threads
and collect data' below)
...
Python code in [[file:sigrokWrapper.py]]. Put this file into one of the
directies named in ~sys.path~ -list (e.g into current working
directory) and use
